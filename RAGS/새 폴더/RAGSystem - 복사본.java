            ...
            try {
                ...
                for (String chunk : chunks) {
                    ...
                    if (embedding != null && !embedding.isEmpty()) {
                        // 실제로는 (Chunk, Embedding) 쌍 전체를 DB에 저장
                        vectorStore.add(chunk); 
                        // 시뮬레이션으로 벡터가 저장된 것처럼 처리
                    }
                }
            } catch (Exception e) {
...
private List<Double> callOllamaApiForEmbedding(String text) throws IOException, InterruptedException {
    ...
    private List<String> retrieveContext(List<Double> queryEmbedding, int topK) {
        // **[핵심 로직]** 실제는 코사인 유사도를 계산해야 하지만, 예제이므로 다음을 반환합니다.
        return Arrays.asList(
            "회사의 새 정책은 2024년 7월 1일부터 적용되며, 팀원들은 이 내용을 필히 숙지해야 합니다.",
            "원격 근무와 관련된 세부 지침은 팀장과의 사전 협의를 통해 결정됩니다. 이는 본 정책의 핵심 요소입니다."
        );
    }
